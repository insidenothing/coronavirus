...
if (isset($_GET['auto']) && empty($_GET['state'])){
	die('only state by state processing');
...
if (isset($_GET['auto']) && isset($_GET['state'])){
	$q = "SELECT zip_code FROM coronavirus_zip where change_percentage_time = '00:00:00' and report_date = '$date' and zip_code <> '$zip' and state_name = '$_GET[state]' order by zip_code ";
	$r = $covid_db->query($q);
	$d = mysqli_fetch_array($r);
	$left = mysqli_num_rows($r);
	if ($left > 0){
...
*/	
	...
	var chartZIPvs2020 = new CanvasJS.Chart("chartContainerZIPvs2020", {
		...
		visible: true,
		showInLegend: true,
		legendMarkerColor: "<?PHP echo $color_2020_line;?>",
		markerColor: "<?PHP echo $color_2020_dot;?>",
		color: "<?PHP echo $color_2020_line;?>",
		yValueFormatString: "#####",
		...
		visible: true,
		showInLegend: true,
		legendMarkerColor: "<?PHP echo $color_2021_line;?>",
		markerColor: "<?PHP echo $color_2021_dot;?>",
		color: "<?PHP echo $color_2021_line;?>",
		yValueFormatString: "#####",
