...
include_once('menu.php');
	...
	<div class="row">
	  <div class="col-sm-6">
		  ...
		  <ol>  
		  <?PHP
		    $q = "SELECT * FROM coronavirus_zip where report_date = '$date' and trend_duration > '13' and (trend_direction = 'DOWN' or trend_direction = 'FLAT' ) order by $sort desc";
		    $r = $covid_db->query($q);
	  ...
	  <div class="col-sm-6">
		  ...
		  <ol> 
		    <?PHP
		    $q = "SELECT * FROM coronavirus_zip where report_date = '$date' and trend_duration > '13' and trend_direction = 'UP' order by $sort desc ";
		    $r = $covid_db->query($q);
		    while($d = mysqli_fetch_array($r)){
			    $zip_closed++;
		       echo "<li><a href='zipcode.php?zip=$d[zip_code]'>$d[state_name] $d[zip_code] $d[trend_duration] days $d[trend_direction] at $d[report_count]</a> ($d[change_percentage_time])</li>"; 
		    }
		    $q = "SELECT * FROM coronavirus_zip where report_date = '$date' and trend_duration = '13' order by $sort desc";
		    $r = $covid_db->query($q);
		    while($d = mysqli_fetch_array($r)){
			    $zip_closed++;
		       echo "<li><a href='zipcode.php?zip=$d[zip_code]'>$d[state_name] $d[zip_code] $d[trend_duration] days $d[trend_direction] at $d[report_count]</a> ($d[change_percentage_time])</li>"; 
