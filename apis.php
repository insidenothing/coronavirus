...
function process_link_check($link){
 if ($_SERVER['REMOTE_ADDR'] == '69.250.28.138' && $link != ''){ 
...
if ($_GET['debug']){
  ...
  $api_id = $_GET['debug'];
  $r = $core->query("SELECT id, cache_date_time FROM coronavirus_api_cache where api_id = '$api_id' order by id DESC");
...
while($d = mysqli_fetch_array($r)){
  ...
  echo "<li style='background-color:$color;' title='$d[api_description]'>".process_link_check($d['run_after'])." (# $d[id])(lvl $d[run_order]) $d[last_updated] <u>$d[api_name]</u> $d[api_status] $list or <a target='_Blank' href='$d[api_url]'>SOURCE</a></li>";
  $line = ob_get_clean();
...
while($d = mysqli_fetch_array($r)){
  ...
  echo "<li style='background-color:$color;' title='$d[api_description]'>(# $d[id])(lvl $d[run_order]) $d[last_updated] <u>$d[api_name]</u> $d[api_status] ".process_link_check($d['run_after'])." $list or <a target='_Blank' href='$d[api_url]'>SOURCE</a></li>";
  $line = ob_get_clean();
  $last_update_hour = date('G',strtotime($dX['cache_date_time'])); 
  $this_hour = date('G') + 2;
  if ($last_update_hour > $this_hour){
