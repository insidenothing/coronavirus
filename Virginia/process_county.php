...
if (isset($_GET['delete'])){
	$delete = date('Y-m-d');
	$core->query(" delete from coronavirus_county where report_date = '$delete' and state_name = 'virginia'");
	die('done '.$delete);
}
...
function coronavirus_county($zip,$date,$count){
	...
	if ($d['id'] == ''){
		...
		$q = "insert into coronavirus_county (testing_count,county_name,report_date,report_count,town_name,state_name,trend_direction,trend_duration) values ('$testing','$zip','$date','$count','$town','virginia','$current_trend','$current_duration') ";
	}else{
