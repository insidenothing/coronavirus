...
if (isset($_GET['cache'])){
  ...
  $q = "SELECT raw_response FROM coronavirus_api_cache where api_id = '$id' ";
  $r = $core->query($q);
...
while($d = mysqli_fetch_array($r)){
  ...
  echo "<li style='background-color:$color;' title='$d[api_description]'>$d[run_order]: $d[last_updated] <u>$d[api_name]</u> $d[api_status]</li>";
  $url = $d['api_url'];
  $id = $d['id'];
  $name = $d['api_name'];
  $r2 = $core->query("SELECT id, cache_date_time FROM coronavirus_api_cache where api_id = '$id' order by id DESC");
  while($d2 = mysqli_fetch_array($r2)){
    echo "<li>$d2[id]: $d2[cache_date_time] <a target='_Blank' href='?cache=$d2[id]'>SOURCE</a></li>";
  }
}
echo "</div>";
