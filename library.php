...
while($d = mysqli_fetch_array($r)){
 ...
 // echo "<h3 style='background-color:$color;' title='$d[api_description]'>$d[run_order]: $d[last_updated] <u>$d[api_name]</u> $d[api_status]</h3>";
  ...
  $raws .= '<div id="list-example2" class="list-group2">';
  while($d2 = mysqli_fetch_array($r2)){
    echo "<a class='btn btn-warning' data-target='#list-example2' data-toggle='collapse' href='#multiCollapseExamplecache$d2[id]' role='button' aria-expanded='false' aria-controls='multiCollapseExamplecache$d2[id]'>$d2[cache_date_time]</a>";
