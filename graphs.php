...
function make_county_prediction($county,$start,$count,$dt){
	...
	$r_int = number_format($r, 0, '.', ',');
		...
		$peak_str[$county] = "<p>On $out $from_now $county peaked at $r_int<p>";
        }
	if (intval($today[$county]) > intval($r) && intval($r) != 0 && $normal[$county] == ''){
		...
		$interval = $datetime1->diff($datetime2);
		$from_now = $interval->format('%R%a days');
		$normal[$county] = "<p style='background-color:pink; '>On $out $from_now $county went under ".$today[$county]." to $r_int</p>";    
	}
	$day_buffer++;
    }
    return $return;
