...
function coronavirus_county($zip,$date,$count){
	...
	$q2 = "select * from coronavirus_county where county_name = '$zip' and report_date = '$date2' and state_name = 'maryland'";
	...
	if ($d['id'] == ''){
		...
		$q = "update coronavirus_county set testing = '$testing', report_count = '$count', trend_direction = '$current_trend', trend_duration = '$current_duration', town_name = '$town'  where county_name = '$zip' and state_name='maryland' and report_date = '$date' ";	
	}
	$covid_db->query($q);
...
if($global_date == date('Y-m-d') || isset($_GET['id'])){
	...
	if(isset($_GET['id'])){
		...
		$r = $covid_db->query("select * from coronavirus_api_cache where api_id = '20' order by id desc limit 0, 1"); // always get the latest from the cache	
	}
