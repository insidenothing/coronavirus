...
if(isset($_GET['validate'])){
  $sms = $_GET['validate'];
  $rSMS = $covid_db->query("SELECT sms_status FROM coronavirus_sms where sms_number = '$sms' ");
  $dSMS = mysqli_fetch_array($rSMS);
  if ($dSMS['sms_status'] != 'confirmed'){
    $covid_db->query("update coronavirus_sms set sms_status = 'confirmed' where sms_number = '$sms' ");
...
if(isset($_GET['phone'])){
  ...
  $covid_db->query("insert into coronavirus_sms ( sms_number ) values ('$sms') ");
